...
const ReactDOM = { createRoot, createPortal };
    ...
    const I = (path, size = 14) => React.createElement('svg', { width: size, height: size, viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 1.8, strokeLinecap: 'round', strokeLinejoin: 'round' }, path);
    ...
    const IconBtn = ({ onClick, title, color = 'var(--tw-slate)', hoverColor = 'var(--tw-ink)', hoverBg = 'var(--tw-bg)', children, danger = false }) => (
    ...
    function getProspectRealStatus(affaires, devisList) {
    ...
    function CampagnesPage({ user, API_URL, onClose }) {
    ...
    // ══════════════════════════════════════════════════
    function NavTabBar({ currentView, onRestore, onOpenPalette }) {
    ...
    // ══════════════════════════════════════════════════
    function ListesView({ type, prospects, user, API_URL }) {
    ...
    function RecapModal({ commercial, period, prospects, onClose, onNavigate, user, API_URL }) {
    ...
    function Header({ user, onLogout, onDashboard, onSuivi, isDashboard, onSettings, onAttribution, showAttribution, onCampagnes, showCampagnes, onListe, activeListe, prospects, onSelectProspect, onNewProspect }) {
    ...
    // ══════════════════════════════════════════════════
    function AttributionView({ prospects, users, user, API_URL, onClose, onUpdateProspect }) {
    ...
    // ══════════════════════════════════════════════════
    function SuspectsNonAttribuesPanel({ API_URL, token }) {
      const [loading, setLoading] = React.useState(false);
    ...
    // ══════════════════════════════════════════════════
    function ImportPanel({ API_URL, token }) {
    ...
    function DashboardConsultant({ prospects, user, prospectActionsInfo, onSelectProspect, onOpenDashboard, API_URL }) {
      const today = new Date(); today.setHours(0,0,0,0);
    ...
    // ═══════════════════════════════════════════════════════════════════
    function CommercialEditor({ selectedProspect, users, user, API_URL, onUpdateProspect }) {
      const [open, setOpen] = React.useState(false);
    ...
    function ActionCompleteModal({ action, prospectId, API_URL, token, onClose, onCompleted }) {
      ...
      const validate = async () => {
        ...
        try {
          ...
          if (createNext && nextDate) {
            const url = action.affaire_id ? `${API_URL}/affaires/${action.affaire_id}/next_actions` : `${API_URL}/prospects/${action.prospect_id || prospectId}/next_actions`;
            await fetch(url, { method: 'POST', headers: hdrs, body: JSON.stringify({ action_type: nextType, planned_date: nextDate, priority: nextPriority, actor: action.actor || '', contact: action.contact || '' }) });
          }
    ...
    function MotifPerteField({ devisId, affaireId, initialValue, onSave }) {
